...
from Breather.utilities.plot import plot_loop_solition
from lightning.pytorch.callbacks import EarlyStopping

# Define training hyperparameters
max_epochs = 16000  # Maximum number of epochs
device = 'mps'  # Training device
early_stopping, patience = False, 2500  # Early stopping settings
...
def read_data_fn(root_path: str):
    ...
    """
    data = pinnstorch.utils.load_data(root_path, "Breather.mat")
    ...
    # noise_level = 0.001
    u = data["xx"].real.T
    v = np.zeros_like(data["xx"].imag.T)
    p = data["qq"].real.T
    q = data["qq"].imag.T
...
hyperparams = {
    ...
    "sampling_points": {
        ...
        "N_b": N_b,
        "N_f": N_f,
        "mesh": (len(mesh.spatial_domain[0].flatten()),len(mesh.time_domain.flatten()))  # Time coordinates
    },
    "training": {
