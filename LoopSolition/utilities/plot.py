...
def plot_loop_solition(mesh, preds):
    ...
    # 从预测数据中已得出：
    xx_pred = preds["xx"].view(256, 128).numpy()
    qq_pred = preds["qq"].view(256, 128).numpy()

    xx_pred_real = xx_pred.real
    qq_pred_modulus = np.abs(qq_pred)
    qq_pred_real = qq_pred.real
...
# #### 绘制模的热图
    ...
    # xx 模的原始值
    plt.subplot(2, 2, 1)
    plt.imshow(xx_real, cmap='viridis', extent=[t.min(), t.max(), y.min(), y.max()], aspect='auto', origin='lower')
    plt.colorbar()
    plt.title('Re(x) Exact')

    # xx 模的预测值
    plt.subplot(2, 2, 2)
    plt.imshow(xx_pred_real, cmap='viridis', extent=[t.min(), t.max(), y.min(), y.max()], aspect='auto', origin='lower')
    plt.colorbar()
    plt.title('Re(x) Predicted')

    # qq 模的原始值
    plt.subplot(2, 2, 3)
    plt.imshow(qq_modulus, cmap='viridis', extent=[t.min(), t.max(), y.min(), y.max()], aspect='auto', origin='lower')
    plt.colorbar()
    plt.title('|qq| Exact')
