...
def xx(y, t):
    ...
    """
    numerator = (
            (8 * 1j * t ** 2 + (24 * 1j * y - 32 * 1j) * t + 72 * 1j * y ** 2 - 48 * 1j * y + 64 * 1j) * np.sqrt(3)
            - 3 * t ** 3
            + (3 * y + 12) * t ** 2
            + (9 * y ** 2 - 30 * y - 120) * t
            + 108 * y ** 3 - 72 * y ** 2 - 48 * y + 192
    )

    # 分母部分
    denominator = (
            24 * t ** 2 + (72 * y - 96) * t + 216 * y ** 2 - 144 * y + 192
    )
...
def qq(y, t):
    ...
    """
    exp_part = np.exp(-1j / 6 * np.sqrt(3) * (t - 6 * y))

    numerator = (
            exp_part * (
            ((-t ** 2 + (-3 * y + 4) * t - 9 * y ** 2 - 6 * y + 8) * np.sqrt(3))
            + 1j * t ** 2
            + (3 * 1j * y - 4 * 1j) * t
            + 9 * 1j * y ** 2 - 42 * 1j * y - 8 * 1j
    )
    )

    # 分母部分
    denominator = 2 * (1j - np.sqrt(3)) * (t ** 2 + (3 * y - 4) * t + 9 * y ** 2 - 6 * y + 8)
...
# Generate grid data
y = np.linspace(-10,10, 256)  # Spatial coordinate range [-1.5, 1.5]
t = np.linspace(-20,20, 256)  # Time coordinate range [-1.5, 1.5]
Y, T = np.meshgrid(y, t)
...
output_path = os.path.join(data_dir, 'RogueWave.mat')
